...
def decode_latents_scheduler(latents, num_frames, decode_chunk_size=8, vae=None, device=None):
    ...
    # vae.decoder.cpu().to(dtype=torch.float32)
    # latents = latents.cpu().to(dtype=torch.float32)
    vae.decoder.cpu()
    ...
    for i in range(0, latents.shape[0], decode_chunk_size):
        ...
        # print(i)
        # print("frame_1.requires_grad", frame.requires_grad)
        frames.append(frame.cpu())
        # print("frame_2.requires_grad", frame.requires_grad)

    # frames = torch.cat(frames, dim=0).to(device=device)
    frames = torch.cat(frames, dim=0)

    # print("latents.requires_grad", latents.requires_grad)
    # decode_kwargs = {}
    # if accepts_num_frames:
    #     decode_kwargs["num_frames"] = num_frames
    # frames = vae.decode(latents, **decode_kwargs).sample
    # print("frames.requires_grad", frames.requires_grad)
...
class EulerDiscreteSchedulerPro(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        refine_reference_face_embeddings=None,
        vae=None,
        decode_chunk_size=4,
        image_processor=None,
        output_type=None,
        device=None,
        target_face_mask_list=None,
        ...
        if refine_face:
            ...
            org_face_embedding = refine_reference_face_embeddings
            org_face_embedding = org_face_embedding.repeat(sample.size()[1], 1)
            z0 = pred_original_sample.clone().detach()  # [1, 16, 4, 64, 64]
            optimizer = torch.optim.Adam([z0], lr=learning_rate)
            cosine_loss = torch.nn.CosineSimilarity(dim=1)
            num_frames = sample.size()[1]
            z0.requires_grad = True
            # initial_z0_parameter = z0.detach().clone()
            for idx in range(num_optimization_iter):
                print("current iteration: ", idx)
                final_face_loss = 0.0
                pred_frames = decode_latents_scheduler_new(latents=z0, num_frames=num_frames, decode_chunk_size=decode_chunk_size, vae=vae, device=device)
                ...
                for k in range(num_frames):
                    target_y_min = target_face_mask_list[k][0]
                    target_y_max = target_face_mask_list[k][1]
                    target_x_min = target_face_mask_list[k][2]
                    target_x_max = target_face_mask_list[k][3]
                ...
                final_face_loss = (1 - cosine_loss(pred_face_embedding, org_face_embedding)).abs().mean()
                optimizer.zero_grad()
                final_face_loss.backward(retain_graph=True)
                optimizer.step()
